...
def test_convert_dataset(tmp_path):
    ...
    stats = convert_dataset(
        images_dir, labels_dir, tmp_path, mode="merged", dry_run=True
    )
    ...
    assert (
        stats["valid_annotations"] == 2
    )  # The car and the clipped box (if mapped correctly)
    ...
    # Test actual run
    stats = convert_dataset(
        images_dir, labels_dir, tmp_path, mode="merged", dry_run=False
    )
