...
from src.aeronetra.detection.types import CountSummary, Detection
...
def draw_detections(image: np.ndarray, detections: list[Detection], color: tuple[int, int, int] = (0, 255, 0), thickness: int = 2) -> np.ndarray:
    ...
    for d in detections:
        ...
        cv2.putText(out_img, label, (x1, y1 - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
...
def export_to_csv(detections: list[Detection], output_path: Path):
    ...
    with open(output_path, 'w', newline='') as f:
        ...
        for d in detections:
            writer.writerow([d.class_id, d.class_name, f"{d.confidence:.4f}", d.box.xmin, d.box.ymin, d.box.xmax, d.box.ymax])
...
def draw_count_summary(image: np.ndarray, summary: CountSummary) -> np.ndarray:
    ...
    for line in text_lines:
        cv2.putText(out_img, line, (20, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
        y_offset += 30
